...
namespace Procurement_Inventory_System
{
    public partial class ViewInvoiceWindow : Form
    {
        ...
        public void LoadSummaryInvoiceDetails()
        {
            ...
            if(Convert.ToDouble(label24.Text) == 0)
            {
                taxRate.Text = "0%";
            }
            else
        ...
        public void LoadBreakdownInvoiceDetails()
        {
            ...
            string query = $"select distinct Item_List.item_name as [Item Name], Purchase_Request_Item.remarks as [Remarks], Purchase_Request_Item.item_quantity as [QTY], \r\nItem_Quotation.unit_price as [Unit Price], Purchase_Order_Item.total_price as [Total Price] from Invoice \r\nINNER JOIN Purchase_Order ON Purchase_Order.purchase_order_id = Invoice.purchase_order_id \r\nINNER JOIN Purchase_Order_Item ON Purchase_Order_Item.purchase_order_id = Purchase_Order.purchase_order_id \r\nINNER JOIN Purchase_Request_Item ON Purchase_Request_Item.purchase_request_item_id = Purchase_Order_Item.purchase_request_item_id \r\nINNER JOIN Item_List ON Item_List.item_id = Purchase_Request_Item.item_id \r\nINNER JOIN Item_Quotation ON Item_Quotation.item_id = Item_List.item_id AND Purchase_Order_Item.order_item_status = 'DELIVERED' \r\nWHERE Invoice.invoice_id = '{InvoiceID.InvID}' AND Item_Quotation.quotation_id = Purchase_Request_Item.quotation_id";
